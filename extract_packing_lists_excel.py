...
def extract_summary_sheet(file_path):
    ...
    try:
        ...
        target = next((s for s in sheet_names if s.strip().lower() == "summary sheet"), None)
        if not target:
...
def save_file_to_order_folder(source_file_path, order_no, output_base_dir):
    """
    Save the original file to a folder named after the order number.
    Maintains the original filename.
    """
    try:
        # Create order folder if it doesn't exist
        order_folder = os.path.join(output_base_dir, order_no)
        ...
        # Destination path
        destination_path = os.path.join(order_folder, original_filename)
        
        # Copy the file
        shutil.copy2(source_file_path, destination_path)
        print(f"✅ Saved {original_filename} to {order_no}/ folder")
        
        return destination_path
    except Exception as e:
        print(f"❌ Error saving file for order {order_no}: {e}")
        return None

# -------- Main Driver -------- #

def extract_packing_lists_excel(input_dir=None, out_dir=None):
    # Use default values if not provided
    if input_dir is None:
        input_dir = "/home/pritom/Desktop/C&A Packing List Extractor/Uploads"
    if out_dir is None:
        out_dir = "/home/pritom/Desktop/C&A Packing List Extractor/media"
    ...
    for file in os.listdir(input_dir):
        if file.lower().endswith((".xls", ".xlsx", ".xlsm")):
            ...
            row = extract_summary_sheet(file_path)
            if row:
                # Save the file to the order folder
                save_file_to_order_folder(file_path, row["order_no"], out_dir)
                all_rows.append(row)
...
if __name__ == "__main__":
    ...
    df = extract_packing_lists_excel(
        input_dir="/home/pritom/Desktop/C&A Packing List Extractor/Uploads",
        out_dir="/home/pritom/Desktop/C&A Packing List Extractor/media"
    )